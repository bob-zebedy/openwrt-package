...
function index()
	...
	entry({"admin", "network", "tcpdump"}, template("tcpdump"), _("Tcpdump"), 70).dependent = false

	page = entry({"admin", "network", "tcpdump", "capture_start"}, call("capture_start"), nil)
	page.leaf = true

	page = entry({"admin", "network", "tcpdump", "capture_stop"}, call("capture_stop"), nil)
	page.leaf = true

	page = entry({"admin", "network", "tcpdump", "update"}, call("update"), nil)
	page.leaf = true

	page = entry({"admin", "network", "tcpdump", "capture_get"}, call("capture_get"), nil)
	page.leaf = true

	page = entry({"admin", "network", "tcpdump", "capture_remove"}, call("capture_remove"), nil)
	page.leaf = true
...
function capture_get(file_type, cap_name)
	if file_type == "all" then
		...
		local tar_captures_cmd = "tar -c " .. tcpdump_cap_folder .. "*.pcap 2>/dev/null"
		local reader = system.ltn12_popen(tar_captures_cmd)
		luci.http.header('Content-Disposition', 'attachment; filename="captures-%s.tar"' % {os.date("%Y%m%d%H%M%S")})

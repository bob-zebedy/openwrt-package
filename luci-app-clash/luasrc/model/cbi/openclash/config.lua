...
HTTP.setfilehandler(function(meta, chunk, eof)
    ...
    if not fd then
        ...
        elseif fp == "proxy-provider" then
            if meta and chunk then
                fd = nixio.open(proxy_pro_dir .. meta.file, "w")
            end
        elseif fp == "rule-provider" then
            if meta and chunk then
                fd = nixio.open(rule_pro_dir .. meta.file, "w")
            end
        elseif fp == "backup-file" then
            if meta and chunk then
                fd = nixio.open(backup_dir .. meta.file, "w")
            end
        end
...
btndl.write = function(a, t)
    ...
    HTTP.header('Content-Disposition', 'attachment; filename="%s"' % {
        sFile
    })
...
if not conf then
    conf = "/etc/openclash/config/config.yaml"
end
